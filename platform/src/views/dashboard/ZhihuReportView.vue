<template>
  <div class="z-page">
    <div class="pg-header">
      <div><h1 class="pg-title">投放实时数据</h1><p class="pg-sub">每小时更新，延迟约 3–4 小时，仅供参考，不作为结算依据</p></div>
      <button class="btn-accent-sm" :disabled="store.loading" @click="store.fetchReport()">
    ...
    <div class="table-card">
      <a-table :data-source="store.data" :columns="cols" :loading="store.loading"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'search_num'">{{ record.fields_data?.search_num ?? '—' }}</template>
          <template v-if="column.key === 'order_num'">{{ record.fields_data?.order_num ?? '—' }}</template>
          <template v-if="column.key === 'created_at'">{{ record.fields_data?.created_at ?? '—' }}</template>
        </template>
      </a-table>
    </div>
  </div>
...
const cols = [
  ...
  { title: '搜索量',   key: 'search_num',          width: 100, align: 'right' as const },
  { title: '订单量',   key: 'order_num',            width: 100, align: 'right' as const },
...
.btn-accent-sm:hover:not(:disabled) { background: var(--color-accent-hover); box-shadow: var(--shadow-glow); }
