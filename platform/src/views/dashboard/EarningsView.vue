<template>
  <div class="earnings-view">
    <div class="page-header">
      ...
      <button class="btn-primary" disabled title="资金链未开放">
        <svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
    ...
    <!-- Withdraw Modal: kept unavailable until all financial gates close. -->
    <a-modal
    ...
    >
      <div class="withdraw-form">
        ...
        <a-form layout="vertical" style="margin-top: 20px">
          <a-form-item label="提现方式" disabled>
          ...
          <a-form-item label="提现金额" disabled>
            <a-input-number
              disabled
              placeholder="资金链未开放"
              style="width: 100%"
        ...
        <div class="tip-text">资金链未开放，暂不可提现或承诺到账时间</div>
      </div>
    </a-modal>
  </div>
</template>
...
const stats = ref([
  ...
  { label: "待结算", value: "—", trend: "状态以审核为准", trendClass: "" },
  { label: "可提现", value: "—", trend: "资金链未开放", trendClass: "" },
]);
...
const filteredRecords = computed(() => {
  ...
  if (searchKey.value)
    res = res.filter(
      (r) =>
        r.keyword.includes(searchKey.value) ||
        r.channelName.includes(searchKey.value),
    );
  return res;
});
...
function initChart(trendData: Array<{ date: string; earnings: number }>) {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      formatter: (p: any) => `${p[0].name}<br/>收益 ${fmtFen(p[0].value)}`,
    },
    xAxis: {
      type: "category",
    ...
    series: [
      {
        name: "收益",
        type: "bar",
        data: trendData.map((p) => p.earnings),
        itemStyle: {
...
async function loadData() {
  try {
    ...
    stats.value = [
      {
        label: "累计收益",
        value: fmtFen(s.total ?? 0),
        trend: "历史总计",
        trendClass: "",
      },
      {
        label: "已结算",
        value: fmtFen(s.settled ?? 0),
        trend: "已完成结算",
        trendClass: "",
      },
      {
        label: "待结算",
        value: fmtFen(s.pending ?? 0),
        trend: "状态以审核为准",
        trendClass: "",
      },
      {
        label: "已锁定",
        value: fmtFen(s.locked ?? 0),
        trend: "锁定中",
        trendClass: "",
      },
      { label: "可提现", value: "—", trend: "资金链未开放", trendClass: "" },
