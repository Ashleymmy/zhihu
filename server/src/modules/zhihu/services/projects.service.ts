...
export async function createProject(
  ...
  if (isDevDemoAuthUser(user)) {
    return {
      ...
      signMethod: input.signMethod ?? 'hmac_sha256',
      isEnabled: true,
...
export async function updateProject(
  ...
  if (isDevDemoAuthUser(user)) {
    return {
      id: projectId,
      name: input.name ?? '知乎 KOC 本地演示项目',
      slug: 'zhihu',
      apiBaseUrl: input.apiBaseUrl ?? 'https://open.zhihu.com',
      signMethod: input.signMethod ?? 'hmac_sha256',
      isEnabled: input.isEnabled ?? true,
      configJson: input.configJson ?? { demo: true },
