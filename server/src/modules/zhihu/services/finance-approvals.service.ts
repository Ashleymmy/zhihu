...
import {
  applyDevDemoWithdrawal,
  cancelDevDemoAppeal,
  cancelDevDemoWithdrawal,
  decideDevDemoAppeal,
  decideDevDemoWithdrawal,
  getDevDemoInvoiceMeta,
  getDevDemoStatement,
  isDevDemoAuthUser,
  listDevDemoAppeals,
  listDevDemoWithdrawals,
  reviewDevDemoAppeal,
  reviewDevDemoWithdrawal,
  submitDevDemoAppeal,
  uploadDevDemoInvoice,
} from '../dev-demo';
...
export async function applyWithdrawal(
  ...
  if (isDevDemoAuthUser(user)) return applyDevDemoWithdrawal(user, input);
...
export async function cancelWithdrawal(user: AuthUser, id: string, ip?: string) {
  if (isDevDemoAuthUser(user)) return cancelDevDemoWithdrawal(user, id);
...
export async function reviewWithdrawal(user: AuthUser, id: string, action: 'approve' | 'reject', remark: string | null, ip?: string) {
  if (isDevDemoAuthUser(user)) return reviewDevDemoWithdrawal(user, id, action, remark);
...
export async function decideWithdrawal(user: AuthUser, id: string, action: 'approve' | 'reject', remark: string | null, ip?: string) {
  if (isDevDemoAuthUser(user)) return decideDevDemoWithdrawal(user, id, action, remark);
...
export async function submitAppeal(
  ...
  if (isDevDemoAuthUser(user)) return submitDevDemoAppeal(user, input);
...
export async function cancelAppeal(user: AuthUser, id: string, ip?: string) {
  if (isDevDemoAuthUser(user)) return cancelDevDemoAppeal(user, id);
...
export async function reviewAppeal(user: AuthUser, id: string, action: 'approve' | 'reject', remark: string | null, ip?: string) {
  if (isDevDemoAuthUser(user)) return reviewDevDemoAppeal(user, id, action, remark);
...
export async function decideAppeal(
  ...
  if (isDevDemoAuthUser(user)) return decideDevDemoAppeal(user, id, action, remark, adjustAmount);
...
export async function uploadInvoice(
  ...
  if (isDevDemoAuthUser(user)) {
    return uploadDevDemoInvoice(user, id, String(file.originalname ?? 'demo-invoice.pdf'));
  }
...
export async function getInvoice(user: AuthUser, id: string) {
  if (isDevDemoAuthUser(user)) {
    ...
    fs.mkdirSync(INVOICE_DIR, { recursive: true });
    const fullPath = path.join(INVOICE_DIR, `dev-demo-${id}.txt`);
    if (!fs.existsSync(fullPath)) fs.writeFileSync(fullPath, `本地演示发票占位文件：${meta.name}`);
    return { path: fullPath, name: meta.name };
  }
