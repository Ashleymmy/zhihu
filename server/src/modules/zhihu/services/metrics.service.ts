...
import {
  devDemoMetricsByKeyword,
  devDemoMetricsByMember,
  devDemoMetricsOverview,
  devDemoTrend,
  isDevDemoAuthUser,
} from '../dev-demo';
...
const range = (query: Record<string, unknown>) => ({
  from: String(query.from ?? '1970-01-01'),
  to: String(query.to ?? '2999-12-31'),
});

export async function overview(user: AuthUser) {
  if (isDevDemoAuthUser(user)) return devDemoMetricsOverview();
  ...
  const [row] = await rows<RowDataPacket & Record<string, number>>(
    `SELECT
     ...
     FROM daily_metrics m WHERE ${scope.clause}`,
    scope.bindings,
  );
  return {
    today: {
      impressions: Number(row?.today_impressions ?? 0),
      clicks: Number(row?.today_clicks ?? 0),
      conversions: Number(row?.today_conversions ?? 0),
      earning: Number(row?.today_earning ?? 0),
    },
    total: {
      impressions: Number(row?.total_impressions ?? 0),
      clicks: Number(row?.total_clicks ?? 0),
      conversions: Number(row?.total_conversions ?? 0),
      earning: Number(row?.total_earning ?? 0),
    },
  };
}

export async function trend(user: AuthUser, query: Record<string, unknown>) {
  if (isDevDemoAuthUser(user)) return devDemoTrend();

  const scope = scopeFilter(user, 'm.owner_id');
  const dates = range(query);
  const data = await rows<
    RowDataPacket & { stat_date: string; impressions: number; clicks: number; conversions: number; earning: number }
  >(
  ...
  return {
    dates: data.map((item) => item.stat_date),
    series: [
...
export async function byKeyword(user: AuthUser, query: Record<string, unknown>) {
  if (isDevDemoAuthUser(user)) return devDemoMetricsByKeyword(query);

  const page = Number(query.page ?? 1);
  const pageSize = Number(query.pageSize ?? 20);
  const dates = range(query);
  const scope = scopeFilter(user, 'm.owner_id');
  const allowedSort = ['earning', 'impressions', 'clicks', 'conversions'];
  const sort = allowedSort.includes(String(query.sort)) ? String(query.sort) : 'earning';
  const order = query.order === 'asc' ? 'ASC' : 'DESC';
  const [count] = await rows<CountRow>(
    `SELECT COUNT(DISTINCT m.channel_id, m.keyword) total FROM daily_metrics m WHERE ${scope.clause} AND m.stat_date BETWEEN ? AND ?`,
    [...scope.bindings, dates.from, dates.to],
  );
  const list = await rows(
    ...
    [...scope.bindings, dates.from, dates.to, pageSize, pageOffset(page, pageSize)],
  );
...
export async function byMember(user: AuthUser, query: Record<string, unknown>) {
  if (isDevDemoAuthUser(user)) return devDemoMetricsByMember(user);
  ...
  return rows(
    `SELECT u.id owner_id, u.display_name, SUM(m.impressions) impressions, SUM(m.clicks) clicks, SUM(m.conversions) conversions, SUM(m.earning) earning
     ...
     GROUP BY u.id, u.display_name ORDER BY earning DESC`,
    [...scope.bindings, dates.from, dates.to],
  );
}

export async function requestSync(user: AuthUser) {
  if (isDevDemoAuthUser(user)) return { jobId: 'dev-demo-metrics', status: 'queued' };

  if (!(await acquireRateLimit(`metrics-sync:${user.sub}`, 600)))
    throw new AppError(429, 42902, '同步过于频繁，请 10 分钟后重试');
  const job = await enqueue(
    'sync-metrics',
    { requestedBy: user.sub },
    { jobId: `metrics-${user.sub}-${Math.floor(Date.now() / 600_000)}` },
  );
