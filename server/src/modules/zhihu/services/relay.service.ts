    ...
    return [
      {
        ...
        status: 'active',
        priority: 10,
        effectiveFrom: new Date(Date.now() - 10 * 86_400_000).toISOString(),
        createdAt: new Date(Date.now() - 10 * 86_400_000).toISOString(),
      },
      {
        id: 'rule-demo-2',
        targetUserId: DEV_DEMO_USER_IDS.leader,
        targetUsername: 'leader',
        targetName: '本地团长',
        targetRole: 'leader',
        method: 'percentage',
        unitPrice: null,
...
export async function createRule(user: AuthUser, input: PricingRuleInput, ip?: string) {
  if (isDevDemoAuthUser(user)) return { id: `rule-demo-${Date.now()}` };
  ...
  if (process.env.DEV_DEMO_AUTH === '1') {
    return [
      ...
      },
      {
        id: 'batch-demo-2',
        title: '8 月下旬演示结算批次',
        periodStart: new Date(Date.now() - 14 * 86_400_000).toISOString().slice(0, 10),
        periodEnd: new Date(Date.now() - 8 * 86_400_000).toISOString().slice(0, 10),
        status: 'approved',
        totalSource: '980.0000',
        totalRelay: '784.0000',
        approvedAt: new Date(Date.now() - 6 * 86_400_000).toISOString(),
...
export async function getBatch(id: string) {
  if (process.env.DEV_DEMO_AUTH === '1') {
    return {
      id,
      title: id === 'batch-demo-2' ? '8 月下旬演示结算批次' : '9 月第一周演示结算批次',
      periodStart: new Date(Date.now() - 7 * 86_400_000).toISOString().slice(0, 10),
      periodEnd: new Date().toISOString().slice(0, 10),
      status: id === 'batch-demo-2' ? 'approved' : 'draft',
      totalSource: '1260.0000',
      totalRelay: id === 'batch-demo-2' ? '784.0000' : '0.0000',
      approvedAt: id === 'batch-demo-2' ? new Date(Date.now() - 6 * 86_400_000).toISOString() : null,
      createdAt: new Date(Date.now() - 2 * 86_400_000).toISOString(),
      items: [
        {
          id: 'settlement-item-demo-1',
          creatorId: DEV_DEMO_USER_IDS.creator,
          creatorUsername: 'creator',
          creatorName: '本地达人',
          sourceAmount: '1260.0000',
          note: '本地演示结算明细',
        },
      ],
      logs: [
        {
          id: 'relay-log-demo-1',
          itemId: 'settlement-item-demo-1',
          earningId: '60001',
          userId: DEV_DEMO_USER_IDS.creator,
          receiverName: '本地达人',
          receiverUsername: 'creator',
          role: 'creator',
          ruleId: 'rule-demo-1',
          method: 'percentage',
          unitPrice: null,
...
export async function createBatch(
  ...
  if (isDevDemoAuthUser(user)) return { id: `batch-demo-${Date.now()}` };
