...
function validatePartNameBytes(nameBytes: Buffer): string {
  ...
  if (ACTIVE_PART_PATTERN.test(name) || (!isDirectory && !ALLOWED_PART_PATTERN.test(name))) return invalid();
  return name;
}
...
function validateExternalAttributes(value: number, madeBy: number, isDirectory: boolean): void {
  if ((value & 0x10) !== 0 && !isDirectory) return invalid();
...
function parseZipEntries(buffer: Buffer): ZipEntry[] {
  ...
  for (let index = 0; index < entriesTotal; index += 1) {
    ...
    const isDirectory = name.endsWith('/');
    validateExternalAttributes(externalAttributes, madeBy, isDirectory);
    if (isDirectory && (method !== 0 || compressedSize !== 0 || uncompressedSize !== 0)) return invalid();
