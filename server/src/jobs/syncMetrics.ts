...
export async function syncMetrics(data: Record<string, unknown>) {
  const from = String(data.from ?? date(7));
  const to = String(data.to ?? date(1));
  ...
  for (const item of items) {
    ...
    const [owner] = await rows<OwnerRow>(
      "SELECT id, owner_id FROM plans WHERE channel_id = ? AND keyword = ? AND status <> 'ended' LIMIT 1",
      [channelId, item.keyword],
    );
