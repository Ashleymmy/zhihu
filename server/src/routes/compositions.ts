...
import { isCompositionCategoryValid, isZonedIsoDateTime, normalizeMediaType, normalizeZonedIsoDateTime, ZHIHU_MEDIA_TYPES } from '../zhihu/composition';

const id = z.string().regex(/^\d+$/);
const mediaType = z.union([
  z.enum(ZHIHU_MEDIA_TYPES),
  z.literal(1),
  z.literal(2),
...
const releaseTime = z.string().refine(
  isZonedIsoDateTime,
  '作品发布时间必须是带时区的 ISO 8601',
...
const input = z.object({
  ...
  (value) => isCompositionCategoryValid(value.compositionType, value.compositionSubType),
  { message: '作品分类组合不正确', path: ['compositionSubType'] },
);
