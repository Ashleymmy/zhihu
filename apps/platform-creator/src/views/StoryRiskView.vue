...
interface RiskWord { type?: number; riskType?: number; keyword: string; channel?: string; createChannel?: string; status?: number; validedAt?: string }
...
const riskTypeFilter = ref<number | ''>('')
...
<template>
  <div class="page-stack">
    ...
    <header class="page-header">
      <div>
        <p class="section-index">06 / 风险举报</p>
        <h1>风险举报</h1>
        <p>截流词与搬运词的风险举报记录，实时来自知乎开放平台。</p>
      </div>
      <div class="page-actions">
        ...
        <select v-model="riskTypeFilter" @change="load">
          <option value="">全部类型</option>
          <option :value="1">截流词</option>
          <option :value="2">搬运词</option>
        </select>
        ...
        <input v-model="keyword" placeholder="搜索风险词" style="width: 140px;" @keyup.enter="load" />
    ...
    <article class="panel data-panel" style="min-height: 300px;">
      <div class="list-toolbar">
        <span class="toolbar-title">风险词列表</span>
      ...
      <div v-else-if="!items.length" class="empty-panel"><span>暂无风险词记录。提交举报需在知乎侧关联作品与截图，如需请联系管理员。</span></div>
      <div v-else class="responsive-table">
        <table>
          <thead><tr><th>风险词</th><th>类型</th><th>归属渠道</th><th>判定状态</th><th>生效时间</th></tr></thead>
          <tbody>
