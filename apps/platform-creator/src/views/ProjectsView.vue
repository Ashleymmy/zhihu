    ...
    <div style="display: grid; grid-template-columns: 240px 1fr; gap: 20px; align-items: start">
      <aside style="display: flex; flex-direction: column; gap: 10px">
        ...
        <button
          ...
          :style="{
            padding: '12px 14px',
            border: `1px solid ${selected?.id === p.id ? 'var(--forest)' : 'var(--line)'}`,
            borderRadius: 'var(--radius)',
            background: selected?.id === p.id ? 'var(--paper)' : 'var(--white)',
            textAlign: 'left',
            cursor: 'pointer',
            fontSize: '12px',
          }"
        ...
        >
          ...
          <div style="font-family: var(--font-mono); font-size: 12px; color: var(--ink-soft)">
            {{ p.slug }}
          </div>
        </button>
      </aside>
        ...
        <article class="panel" style="padding: 20px">
          ...
          >
            ...
            <a
              ...
              :href="c.courseUrl"
              target="_blank"
              style="color: var(--forest); font-size: 12px"
              >查看</a
            >
          </div>
        </article>
      ...
      <div
        ...
        style="color: var(--ink-soft); font-size: 12px; padding: 40px; text-align: center"
      >
        选择一个项目查看详情
      </div>
    </div>
      ...
      >
        ...
        >
          ...
          <form class="form-grid" @submit.prevent="createProject" style="gap: 16px">
            ...
            <div>
              <label>Slug</label><input v-model="form.slug" required maxlength="32" pattern="[a-z0-9-]+" />
            </div>
            <div class="form-submit" style="display: flex; gap: 10px; margin-top: 8px">
              <button type="submit" class="primary-action" :disabled="creating" style="flex: 1">
                {{ creating ? '创建中...' : '确认创建' }}
              </button>
