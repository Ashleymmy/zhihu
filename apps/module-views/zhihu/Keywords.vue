...
const form=reactive({keyword:'',taskId:props.context.options.tasks[0]?.id||'',mappingId:props.context.options.mappings[0]?.id||'',channelId:props.context.options.channels[0]?.id||'',landingUrl:'',popularizeType:1})
...
onMounted(()=>run(async()=>{prices.value=(await props.context.http.get<{list:typeof prices.value}>('/price-agreements',{...props.context.scope,page:1,pageSize:100})).list}))
</script>
...
</div><small>“创建成功”仅表示已取得知乎计划 ID，不代表已核实官方当前生效状态。</small>
