  ...
  <div class="page-stack">
    ...
    <section class="workspace-grid">
      <div class="min-w-0">
        <article class="panel" style="padding: 22px">
          ...
          <div class="sync-list">
            <div class="sync-row">
              <div>
                <strong>渠道目录</strong><small>上次同步：{{ fmt(info?.sync.channels ?? null) }}</small>
              </div>
              <button class="row-action" :disabled="!!syncing" @click="trigger('channels')">
                {{ syncing === 'channels' ? '入队中...' : '立即同步' }}
              </button>
            </div>
            <div class="sync-row">
              <div>
                <strong>推广任务</strong><small>上次同步：{{ fmt(info?.sync.tasks ?? null) }}</small>
              </div>
              <button class="row-action" :disabled="!!syncing" @click="trigger('tasks')">
                {{ syncing === 'tasks' ? '入队中...' : '立即同步' }}
              </button>
            </div>
            <div class="sync-row">
              <div>
                <strong>运营指标</strong><small>上次同步：{{ fmt(info?.sync.metrics ?? null) }}</small>
              </div>
              <button class="row-action" :disabled="!!syncing" @click="trigger('metrics')">
                {{ syncing === 'metrics' ? '入队中...' : '立即同步' }}
              </button>
            </div>
            <div class="sync-row">
            ...
            <div class="sync-row">
              ...
              <button class="row-action" :disabled="!!syncing" @click="trigger('compositionStatus')">
                {{ syncing === 'compositionStatus' ? '入队中...' : '立即同步' }}
              </button>
            </div>
          </div>
        </article>
      ...
      <aside class="workspace-rail">
        ...
        <div class="auth-card">
          <p>
            <span>知乎 OpenAPI</span><strong>{{ info?.zhihuApiBase || '—' }}</strong>
          </p>
          <p>
            <span>凭证模式</span>
            <span :class="['status-badge', info?.zhihuCredentialMode === 'real' ? 'active' : 'ended']">{{
              info?.zhihuCredentialMode === 'real' ? '真实凭证' : 'Mock'
            }}</span>
          </p>
          <p>
            <span>运行环境</span
            ><strong style="font-family: var(--font-mono); font-size: 13px"
              >Node {{ info?.node ?? '—' }}</strong
            >
          </p>
        </div>
