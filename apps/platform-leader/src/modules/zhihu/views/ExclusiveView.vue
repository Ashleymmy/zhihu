...
<template><Workspace :http="http" :core-http="coreHttp" :role="auth.user?.role??''" :user-id="auth.user?.id??''" :parent-id="auth.user?.parentId" :admin-duty="auth.user?.adminDuty" :section="section" :active-tab="String(route.query.tab||'keywords')" @navigate="router.push" /></template>